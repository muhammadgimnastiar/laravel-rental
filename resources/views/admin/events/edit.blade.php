    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="car_id">{{ trans('cruds.event.fields.car') }}</label>
                <select class="form-control select2 {{ $errors->has('car') ? 'is-invalid' : '' }}" name="car_id" id="car_id" required>
                    @foreach($cars as $id => $car)
                        <option value="{{ $id }}" {{ ($event->car ? $event->car->id : old('car_id')) == $id ? 'selected' : '' }}>{{ $car }}</option>
                    @endforeach
                </select>
                @if($errors->has('car'))
                ...
                @endif
                <span class="help-block">{{ trans('cruds.event.fields.car_helper') }}</span>
            </div>
            <div class="form-group">
