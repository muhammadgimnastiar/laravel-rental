...
class BookingsController extends Controller
{
    public function searchCar(Request $request)
    {
        ...
        if($request->filled(['start_time', 'end_time', 'capacity'])) {
            ...
            $cars = Car::where('capacity', '>=', $request->input('capacity'))
        ...
        return view('admin.bookings.search', compact('cars'));
    }

    public function bookCar(Request $request, EventService $eventService)
    {
        $request->merge([
            'user_id' => auth()->id()
        ]);
        ...
        if ($eventService->isCarTaken($request->all())) {
            return redirect()->back()
                    ->withInput()
                    ->withErrors(['recurring_until' => 'This Car is not available until the recurring date you have chosen']);
        }

        if (!auth()->user()->is_admin && !$eventService->chargeHourlyRate($request->all(), $car)) {
            return redirect()->back()
                    ->withInput()
        ...
        return redirect()->route('admin.systemCalendar')->withStatus('A car has been successfully booked');
    }
}
